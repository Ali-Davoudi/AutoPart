...
from basket_order_module.models import UserBasketOrder
from product_module.models import Product, ProductCategory, ProductBrand, ProductGallery, ProductComment, ProductVisit
from site_module.models import SiteBanner
from utils.convertors import group_list
from wishlist_module.models import UserWishList
...
def product_categories_component(request: HttpRequest):
    ...
    return render(request, 'product_module/components/ProductCategoriesComponent.html', context)
...
def product_brands_component(request: HttpRequest):
    ...
    return render(request, 'product_module/components/ProductBrandsComponent.html', context)
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # For product_column number in template that calculate the count of related products to show items correctly.
        # context['related_products_count'] = Product.objects.filter(
        #     brand_id=loaded_product.brand_id, is_active=True,
        #     is_delete=False).exclude(pk=loaded_product.id).all()[:5].count()
        # Above problem in template: product_column{{ related_products_count }}
        # Update: I used slice and length filter for related_products in template for product_column instead of above problem
...
class AddProductComment(View):
    def post(self, request: HttpRequest):
        ...
        return render(request, 'product_module/components/AddProductComment.html', context)


# Searching products
class SearchProductListView(ListView):
    template_name = 'product_module/ProductList.html'  # Affected in header partial
    model = Product
    context_object_name = 'products'
    paginate_by = 2
